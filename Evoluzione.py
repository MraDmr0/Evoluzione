...
class evoluzione:
    ...
    def potenziale(self):
        ...
        if self.D == 2:
            ...
            wr_+ = self.wr[1]+im*self.wr[2]
            
            V = np.zeros((self.D,self.D) , dtype = complex)
            V[1,1] = (self.wr[0]+self.wr[3])*np.cos(self.wp*self.t0)
            V[1,0] = wr_+.conjugate()*0.5*(np.exp(-im*self.t0*(self.wp-self.wl))+np.exp(im*self.t0*(self.wp+self.wl)))
            V[0,1] = wr_+*0.5*(np.exp(im*self.t0*(self.wp-self.wl))+np.exp(-im*self.t0*(self.wp+self.wl)))   
